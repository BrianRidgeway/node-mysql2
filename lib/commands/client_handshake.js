...
class ClientHandshake extends Command {
  ...
  useSha256PasswordAuthPlugin(authParams) {
    return (
  ...
  useSha2PasswordAuthPlugin(authParams) {
    return (
  ...
  receiveServerPublicKey(connection) {
    if (
      this.authPlugin.packetMarker === 1 &&
      isPublicRsaKey(this.authPlugin.pluginMoreData)
    ) {
      if (connection.config.debug) {
  ...
  handleSha2PasswordAuth(connection, packetMarker, authParams) {
    ...
    if (this.password.length === 0) {
      connection.writePacket(new Packets.AuthSwitchResponse('').toPacket());
      ...
      if (this.authPlugin.pluginMoreData.toString('hex') == '04') {
        if (connection.config.debug) {
          console.log(
      ...
      if (connection.isSecure()) {
        ...
        if (
          ...
          this.receiveServerPublicKey(connection)
        ) {
          if (connection.config.debug) {
            console.log(
          ...
          connection.writePacket(
            new Packets.AuthSwitchResponse(Buffer.from('02', 'hex')).toPacket()
          );
        }
      }
    }
  ...
  handleSha256PasswordAuth(connection, packetMarker, authSwitchHandlerParams) {
    ...
    if (connection.isSecure(true)) {
      ...
      connection.writePacket(
        new Packets.AuthSwitchResponse(Buffer.from('01', 'hex')).toPacket()
      );
    }
  }
  ...
  handshakeResult(packet, connection) {
    ...
    if (marker === 0xfe || marker === 1) {
      ...
      } else if (this.useSha256PasswordAuthPlugin(authSwitchHandlerParams)) {
        this.handleSha256PasswordAuth(
          connection,
          marker,
          authSwitchHandlerParams
        );
      } else if (this.useSha2PasswordAuthPlugin(authSwitchHandlerParams)) {
