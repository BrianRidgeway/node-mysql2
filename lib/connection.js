...
class Connection extends EventEmitter {
  ...
  writeAuthSwitchResponsePacket(message) {
    this.writePacket(new Packets.AuthSwitchResponse(message).toPacket());
  }
  writePacket(packet) {
  ...
  isSecure(socketsNotSecure) {
    return (
      this._secureEstablished || (!socketsNotSecure && this.config.socketPath)
    );
  }
  hasServerPublicKey() {
    return (
      this.config.rsa !== undefined &&
      this.config.rsa.serverPublicKey !== undefined
    );
  }
  canAskForPublicKey() {
    return this.config.rsa !== undefined && this.config.rsa.getServerPublicKey;
  }
  pipe() {
      ...
      if (this.sequenceId !== packet.sequenceId) {
        const err = new Error(
          `Warning: got packets out of order. Expected ${
            this.sequenceId
          } but received ${packet.sequenceId}`
        );
