...
class ClientHandshake extends Command {
  ...
  calculateSha256PasswordAuthToken(serverScramble, serverPublicKey) {
    let scrambledPassword = Buffer.from(this.password + '\0', 'ascii');
    for (var i = 0; i < scrambledPassword.length; i++) {
      scrambledPassword[i] ^= serverScramble[i % serverScramble.length];
    }
    let authToken = crypto.publicEncrypt(
      {
        key: serverPublicKey,
